...
data_males <- haven::read_spss("./data/dataE_1402.sav") %>% dplyr::select(gender, CASE, sexdrive2, CSBD, PPCS_6, meffort, socialanx, lon, mvalue, attr) %>% filter(gender == 2) %>% dplyr::select(-gender)
```
...
#Create vector with rownames
Variable <- c("Sex drive", "Compulsive sex. behaviour", "Probl. porn use", "Mating effort", "Social anxiety",
              "Loneliness", "Mate value", "Sex. interest children")
#Bind Variable column to the beginning
desc_m <- cbind(Variable, desc_m[, -1]) %>% 
  slice(8, 1, 2, 3, 4, 7, 5, 6)
