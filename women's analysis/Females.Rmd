...
data_females <- haven::read_spss("./data/dataE_1402.sav") %>% dplyr::select(gender, CASE, sexdrive2, CSBD, PPCS_6, meffort, socialanx, lon, mvalue, attr) %>% filter(gender == 1) %>% dplyr::select(-gender)
```
...
Variable <- c("Sex drive", "Mating effort",  "Social anxiety", "Loneliness", "Mate value", "Sex. interest children", "Compulsive sex. behaviour",
              "Probl. porn use")
#Bind Variable column to the beginning
desc_f <- cbind(Variable, desc_f[, -1]) %>% 
  slice(6, 1, 7, 8, 2, 5, 3, 4)
